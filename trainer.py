...
from data_preprocessing import ABUSset
import timm
import logging
...
def train_one_fold(fold, args, model, logger, train_loader, val_loader, optimizer, loss_fn, scheduler):
    ...
    path='./checkpoints_originset/stage_{}/classes_{}/fold_{}'.format(args.stage, args.num_classes, fold)
...
def train_one_fold_for_six(fold, args, model, logger, train_loader, val_loader, optimizer, loss_fn, scheduler):
    # check if model path exists
    path='./checkpoints_originset/stage_{}/classes_{}/fold_{}'.format(args.stage, args.num_classes, fold)
    if not os.path.exists(path):
        os.mkdir(path)
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            for i,batch in enumerate(val_loader):
                ...
                pre_mask = torch.zeros(outputs.size()).scatter_(1, predicted.cpu().view(-1, 1).long(), 1.)
                predict_num += pre_mask.sum(1)  # 得到数据中每类的预测量
                tar_mask = torch.zeros(outputs.size()).scatter_(1, label.data.cpu().view(-1, 1).long(), 1.)
                target_num += tar_mask.sum(1)  # 得到数据中每类的数量
                acc_mask = pre_mask * tar_mask 
                acc_num += acc_mask.sum(1) # 得到各类别分类正确的样本数量
                print(.shape)
                print(acc_num.shape)
                print(acc_num.shape)
                '''
            ...
            if accuracy>best_val:
                ...
                logger.info(f'Best acc: {best_val:.3f}, Current acc: {accuracy:.3f}')
    # val loop
    logger.info(f"Fold {fold}, best val acc: {best_val}")
    return best_val
