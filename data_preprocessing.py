...
class ABUSsetNew(torch.utils.data.Dataset):
    def __init__(self, args, fold=None, mode='train'):
        if args.stage==2 or args.stage==0:
            self.root=args.imagesTr+f"/stage{args.stage}/fold_{fold}/{mode}"
        else:
            self.root=args.imagesTr+f"/stage{args.stage}_{args.num_classes}classes/fold_{fold}/{mode}"
        self.mode=mode
        ...
        print(f"fold {fold} {mode} set length:", len(self.image_path))
